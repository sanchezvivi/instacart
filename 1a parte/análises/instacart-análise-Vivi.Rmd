...
orders_clusters_gg %>% 
  group_by(cluster, order_dow, order_hour_of_day) %>%
  filter(cluster == 'Rafaela') %>% 
  summarise(orders_n = n()) %>% 
  ggplot(aes(x = order_hour_of_day, y = order_dow, size  = (orders_n/sum(orders_n))))+ 
  geom_density(show.legend = F, color  = instacart_colors['orange']) +
  #geom_hline(aes(yintercept = 10), linetype = 'dashed', show.legend = F) +
  scale_fill_instacart(palette = 'clusters', reverse = F) +
  labs(y = 'Percentual de Pedidos', x = 'Horário da compra', 
       color = 'Cluster') +
  theme(legend.position="bottom")
