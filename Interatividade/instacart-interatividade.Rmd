...
# n_k <- eventReactive(input$submit, {input$k})
...
```{r Cluster_Text, warning=FALSE, error=FALSE}
...
renderPrint({
        ...
        if (is.null(input$plot_dblclick$panelvar1)){
            ...
            print(base_graf_exp %>% select(-user_id) %>% group_by(cluster) %>% summarise(n_compras = mean(n_compras), 
                                                                                         t_mean = mean(t_mean), 
                                                                                         mean_prod_cart = mean(mean_prod_cart), 
                                                                                         mean_peso_cart = mean(mean_peso_cart), 
                                                                                         mean_rec_fat = mean(mean_rec_fat)));
            print(clust_kmean$size)
        } else {
            base_graf_exp <- base_k_user %>%
                              bind_cols(cluster = clust_kmean$cluster)
            ...
            print(base_graf_exp %>% select(-user_id) %>% group_by(cluster) %>% summarise(n_compras = mean(n_compras), 
                                                                                         ...
                                                                                         mean_rec_fat = mean(mean_rec_fat)) %>% 
                    filter(cluster == input$plot_dblclick$panelvar1));
            print(clust_kmean$size[as.numeric(input$plot_dblclick$panelvar1)])
        }
    })
